...
export const PANEL_COPY = {
  ...
  theme: L("Тема", "Theme"),
  sectionOrder: L("Порядок секций", "Section order"),
  sectionOrderEdit: L("Изменить", "Edit"),
  sectionOrderSave: L("Сохранить", "Save"),
  sectionOrderInfo: L(
    "Ручка — перетащить секцию. Пин рядом — закрепить сверху, чтобы секция не уезжала со скроллом. Panel Settings тоже в этом списке. Пресеты — ряд внутри неё.",
    "Grip to drag a section. Pin beside it keeps the section at the top of the scroll. Panel Settings is in the same list. Presets live inside it.",
  ),
