...
Панель настроек сцены для Next.js. **⌘M** — тайминги, easing, цвета и раскладка прямо на странице, без правок в коде. `shortcut={false}` выключает хоткей; шестерёнка по-прежнему открывает панель.
...
- Нижний таймлайн анимации (`SettingsTimeline`) на том же плеере
- Общие кастом-виджеты: чипы, матрица пропусков, shuffle, join обводки
- Русский и English
