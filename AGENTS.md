...
- После изменения схемы панели — обновить описание страницы в `<repo>/AGENTS.md` (дефолты, storage key, что в scrub).
...
**Новые сцены — только через реестр; старые inline-схемы работают, миграция по мере правок.**
...
- `custom` — не новый тип ряда ядра: Пропуски / Мест в строке / Shuffle / Join — виджеты пакета, в схему через `custom`. Plot — `curveSection`. `keys` + `rowReset` — чтобы Reset/Copy и точка-reset работали. Grid Kind Switch — сцена, не пакет.
- `refs` / `derived` не считаются в Reset/Copy (`refs` — исходный ряд один раз; `derived` не хранится). `derived` без `after`/`where` в указке исчезает. `defineParams` не экспортирует голые `number`/`text` — только `param.*`.
...
- Текст: Geist Sans **15/20** — заголовки групп/subsections и лейблы рядов; поля, сегменты и dropdown **14/18** (числа / hex / bezier — Geist Mono, текстовые и сегменты — Geist Sans). Иконки пункта и Section Header — `--sp-fg` (`text/bright`), не `--sp-muted`. Ряд на hover не светлеет (лейбл / иконка / фон); hover только у контрола. В Figma hover на атомах: Field / Hex / Action / Replay / Swatch / Field Stepper `State=Default|Hover`; Color Hex, Dropdown, Scrub Toggle, Toggle — ось `Hover=Off|On` (Toggle Off hover = `border/strong`; On уже strong); Pick Cell `Off|Hover|On`; Stepper / Orient / Fit / Fit Text Align / Segment — **Hover на всём треке** (`border/strong` поверх ячеек, в коде `::after` overlay, не inset box-shadow — заливка ячеек иначе перекрывает обводку). Не на `Panel / Row`.
...
- ⌘M — физический `KeyM` + meta; игнор при фокусе в input/textarea/select/contenteditable. Shortcut открывает/закрывает **без** motion (как command palette); клик по шестерёнке — enter 220 / exit 160 ease-out.
